        ...
        <div className="glass-card p-8 md:p-12 grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="flex flex-col gap-8">
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between">
                <label className="text-sm font-semibold text-[--muted] uppercase tracking-wide">Initial Deposit</label>
                <span className="text-lg font-bold text-white">{formatCurrency(principal)}</span>
              </div>
              <input type="range" id="principal-slider" min={0} max={100000} step={500} value={principal} onChange={(e) => setPrincipal(Number(e.target.value))} aria-label="Initial deposit amount" />
              <div className="flex justify-between text-xs text-[--muted]"><span>$0</span><span>$100K</span></div>
            </div>

            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between">
                <label className="text-sm font-semibold text-[--muted] uppercase tracking-wide">Monthly Contribution</label>
                <span className="text-lg font-bold text-white">{formatCurrency(monthly)}</span>
              </div>
              <input type="range" id="monthly-slider" min={0} max={5000} step={50} value={monthly} onChange={(e) => setMonthly(Number(e.target.value))} aria-label="Monthly contribution amount" />
              <div className="flex justify-between text-xs text-[--muted]"><span>$0</span><span>$5K/mo</span></div>
            </div>

            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between">
                <label className="text-sm font-semibold text-[--muted] uppercase tracking-wide">Annual Return Rate</label>
                <span className="text-lg font-bold text-white">{rate}%</span>
              </div>
              <input type="range" id="rate-slider" min={1} max={20} step={0.5} value={rate} onChange={(e) => setRate(Number(e.target.value))} aria-label="Annual return rate percentage" />
              <div className="flex justify-between text-xs text-[--muted]"><span>1%</span><span>20%</span></div>
            </div>

            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between">
                <label className="text-sm font-semibold text-[--muted] uppercase tracking-wide">Time Horizon</label>
                <span className="text-lg font-bold text-white">{years} yrs</span>
              </div>
              <input type="range" id="years-slider" min={1} max={40} step={1} value={years} onChange={(e) => setYears(Number(e.target.value))} aria-label="Time horizon in years" />
              <div className="flex justify-between text-xs text-[--muted]"><span>1 yr</span><span>40 yrs</span></div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 pt-2 border-t border-[rgba(79,142,247,0.1)]">
              <div>
                ...
                <input id="principal-input" type="number" value={principal} min={0} onChange={(e) => setPrincipal(Math.max(0, Number(e.target.value)))} className={inputClass} />
              </div>
              <div>
                <label className="text-xs text-[--muted] font-medium mb-1.5 block">Monthly ($)</label>
                <input id="monthly-input" type="number" value={monthly} min={0} onChange={(e) => setMonthly(Math.max(0, Number(e.target.value)))} className={inputClass} />
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-8">
            <div className="text-center lg:text-left">
              ...
              <div className="text-5xl md:text-6xl font-extrabold gradient-text mb-4 transition-all duration-300" style={{ fontFamily: "var(--font-plus-jakarta)" }}>
            ...
            <div>
              <p className="text-sm font-semibold text-[--muted] uppercase tracking-widest mb-4">Year-by-Year Growth</p>
                ...
                {chartData.map((val, i) => {
                  ...
                  return (
                    <div
                      ...
                      style={{
                        height: `${heightPct}%`,
                        background: isLast ? "linear-gradient(180deg, #4f8ef7, #2dd4a0)" : `rgba(79,142,247,${0.15 + (i / chartData.length) * 0.35})`,
            ...
            <div>
              <p className="text-sm font-semibold text-[--muted] uppercase tracking-widest mb-3">Contribution vs. Returns</p>
              <div className="h-3 rounded-full overflow-hidden flex">
                <div className="h-full transition-all duration-500" style={{ width: `${result > 0 ? (totalContributed / result) * 100 : 100}%`, background: "rgba(79,142,247,0.6)" }} />
                <div className="h-full flex-1" style={{ background: "linear-gradient(90deg, #2dd4a0, #4f8ef7)" }} />
              </div>
              <div className="flex justify-between text-xs text-[--muted] mt-2">
                <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm bg-[rgba(79,142,247,0.6)] inline-block" />Contributions</span>
                <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm bg-[--success] inline-block" />Returns</span>
              </div>
            </div>
          </div>
        </div>
